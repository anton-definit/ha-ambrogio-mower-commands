"""Lightweight per-IMEI command queue."""
...
class CommandQueue:
    """Serial executor per IMEI with simple retry/backoff and optional re-auth."""

    def __init__(
        self,
        client: AmbrogioClient,
        *,
        rate_delay_sec: float = 0.25,            # small spacing between calls
        max_retries: int = 2,
        on_reauth: Optional[Callable[[], "asyncio.Future | bool"]] = None,
    ) -> None:
        self._client = client
        self._rate_delay = rate_delay_sec
    ...
    async def _run(self, cmd: Command) -> Any:
        # Built-in delay op: params={"seconds": float}
        if cmd.op == "delay":
